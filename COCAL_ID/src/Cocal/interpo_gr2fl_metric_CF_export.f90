...
use COCAL_ID_interface_interpo_gr2fl_export
implicit none
  real(8), pointer :: psi(:,:,:), alph(:,:,:), bvxd(:,:,:), bvyd(:,:,:), bvzd(:,:,:)
  real(8), pointer :: psif(:,:,:), alphf(:,:,:), bvxdf(:,:,:), bvydf(:,:,:), bvzdf(:,:,:), rs(:,:)
 !
  call COCAL_ID_interpo_gr2fl_export(alph, alphf, rs)
  call COCAL_ID_interpo_gr2fl_export(psi , psif , rs)
  call COCAL_ID_interpo_gr2fl_export(bvxd, bvxdf, rs)
  call COCAL_ID_interpo_gr2fl_export(bvyd, bvydf, rs)
  call COCAL_ID_interpo_gr2fl_export(bvzd, bvzdf ,rs)
!
